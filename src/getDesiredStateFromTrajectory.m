...
%我们将通过differential flatness 来得到以下期望值：
%rq（包括xq,zq）, drq（飞机期望速度dxq,dzq）, beta（机械臂与水平的角度）, 
%theta（pitch）, F（推力）, M(产生theta的力矩)
% 
...
% coefficient = reshape(basic_coef(: , 1), 4, 8);           %变成了4行8列的矩阵，第一行是x的系数cx0-
m_q = 2;      %2kg
m_g = 0.2;    %0.2kg
m_s = (m_g + m_q);
...
dF = m_s * (b3 .* d3r_s);
theta = acos(dot(b3, [0; 0; 1]) / (norm(b3) * norm([0; 0; 1]))); % 弧度
